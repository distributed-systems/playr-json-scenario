    ...
    module.exports = new Class({
        ...
        , validate: function(name, data, validator) {
            if (validator.optional && type.undefined(data)) return Promise.resolve();
                ...
                // check the type
                if (type(data) !== validator.type) return Promise.resolve(`${name}: ${type(data)} !== ${validator.type}`);
                else {

                    switch (validator.comperator) {
                        case '=':
                            if (data === validator.value) return Promise.resolve();
                            else return Promise.resolve(`${name}: ${this.toString(data)} !== ${this.toString(validator.value)}`);
                            break;
                        ...
                        case '>':
                            if (validator.type === 'number') {
                                if (data > validator.value) return Promise.resolve();
                                else return Promise.resolve(`${name}: Expected value ${data} to be > ${validator.value}`);
                            }
                            else if (validator.type === 'string'){
                                if (data.length > validator.value) return Promise.resolve();
                                else return Promise.resolve(`${name}.length: Expected value ${data.length} to be > ${validator.value}`);
                            }
                            else return Promise.reject(`${name}: cannot compare ${validator.type} using the operator >`);
                            break;
                        ...
                        default: 
                            return Promise.reject(`${name}: Invalid comperator ${validator.comperator}!`);
                    }
                }
            }
        }
